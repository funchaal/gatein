...
export default function ChatEmptyState() {
  ...
  const appointments = useSelector(selectByType["active-all"]);
  const terminals = useSelector(selectAllTerminals);

  const [suggestions, setSuggestions] = useState([]);

  useEffect(() => {
    const terminalsList = Object.values(terminals || {});
    if (!appointments?.length || !terminalsList.length) return;

    const terminal_ids = new Set(
      appointments.map(app => app.terminal_id)
    );

    const relatedTerminals = terminalsList.filter(t =>
